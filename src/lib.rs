...
/// Configuration for running the extractor from either the CLI binary or another crate.
#[derive(Debug, Clone)]
...
fn find_largest_embedded_jpeg_impl<B: ByteOrder>(raw_buf: &[u8]) -> Result<EmbeddedJpegInfo> {
...
async fn process_directory(
    ...
    for in_path in entries {
        ...
        let output_dir = Arc::clone(&out_dir);
        let path_for_display = in_path.clone();
        let task: tokio::task::JoinHandle<Result<ProcessingResult>> = tokio::spawn(async move {
            let permit = semaphore.acquire_owned().await?;
            let result = process_file(in_path, output_dir, relative_path).await;
            drop(permit);
            progress_bar.inc(1);
            Ok(ProcessingResult {
                result,
...
/// Extract embedded JPEGs from the configured RAW directory into the output directory.
